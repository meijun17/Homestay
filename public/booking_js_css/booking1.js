...
$(function () {
    ...
    checkItem.onclick = function () {
        ...
        if (parseInt(adult) < 3 && parseInt(child) == 0) {
            document.getElementById("totalPrice").innerText = 2980 * parseInt(planDays) + '元';
        } else if (parseInt(adult) < 3) {

        }
        else {
            document.getElementById("totalPrice").innerText = 2980 * parseInt(planDays) + 500 * parseInt(planDays) + '元';
        }
    }
});
...
function childfun() {
    ...
    var str = "<input>";
    if (childNum > 0) {
        str = `<input type="checkbox">8歲以上(含8歲)`;

        // str += `<option value ="1">8歲以上(含8歲)</option>`;
        // str += `<option value ="0">8歲以下</option>`;
    }

    document.getElementById('childcheckbox').innerHTML = str;

    var childyears = $('#childcheckbox').val();
    console.log(childyears);
}
